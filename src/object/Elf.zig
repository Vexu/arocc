...
const additional_sections = 3; // null section, shstrtab, symtab
...
pub fn getSection(elf: *Elf, section_name: []const u8) !*std.ArrayListUnmanaged(u8) {
    const sect_idx = elf.sections.getIndex(section_name) orelse blk: {
    ...
    const sect = &elf.sections.values()[sect_idx];
    return &sect.data;
}
...
pub fn declareSymbol(
    ...
    try elf.strtab.writer().print("{s}\x00", .{name});

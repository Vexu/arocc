...
fn diagnoseField(
    ...
    if (res.val.opt_ref == .none) {
        ...
        try p.err(arg_start, .attribute_arg_invalid, .{ expectedArgType(Wanted), "expression" });
        return true;
    }
    ...
    try p.err(arg_start, .attribute_arg_invalid, .{ expectedArgType(Wanted), switch (key) {
        .int => "int",
        .bytes => "string",
        .float => "float",

    ...
    inline for (comptime std.meta.fieldNames(@TypeOf(args))) |arg_name| {
        ...
        i += switch (@TypeOf(arg)) {
            ...
            *Result => try p.formatResult(w, fmt[i..], arg.*),
            ...
            attributes.Namespaced => try formatNamespaced(w, fmt[i..], arg),
            @import("Attribute.zig") => 0, // TODO
...
fn formatNamespaced(w: *std.Io.Writer, fmt: []const u8, name: attributes.Namespaced) !usize {
...
fn decl(p: *Parser) Error!bool {
    ...
    var init_d = (try p.initDeclarator(&decl_spec, decl_node)) orelse {
        ...
        for (p.wip_attrs.buf.items[p.wip_attrs.top..]) |wip_attr| {
            try p.err(wip_attr.tok, .ignored_record_attr, .{ wip_attr.name, base_tag_name });
        }
...
fn attribute(p: *Parser, syntax: attributes.Syntax) Error!void {
    const first_tok = p.tok_i;
    const namespaced = blk: {
        ...
        if (attributes.Namespaced.fromString(syntax, opt_namespace, name)) |some| break :blk some;
        if (syntax == .declspec) {
    ...
    try p.wip_attrs.buf.append(gpa, .{
        .name = namespaced,
        .syntax = syntax,
...
fn typeName(p: *Parser) Error!?QualType {
    ...
    return try p.wip_attrs.applyTypeAttrs(p, qt);
}

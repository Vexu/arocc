...
computed_goto_tok: Token = Token.invalid,
...
record: struct {
    ...
    flexible_field: Token = Token.invalid,
    start: usize = 0,
    field_attr_start: usize = 0,

    fn addField(r: @This(), p: *Parser, name: StringId, tok: Token) Error!void {
    ...
    fn addFieldsFromAnonymous(r: @This(), p: *Parser, ty: Type) Error!void {
        for (ty.data.record.fields) |f| {
            ...
            } else if (f.name_tok.id != .invalid) {
                try r.addField(p, f.name, f.name_tok);
            }
        }
    }
} = .{},
record_members: std.ArrayListUnmanaged(struct { tok: Token, name: StringId }) = .{},
@"switch": ?*Switch = null,
in_loop: bool = false,
pragma_pack: ?u8 = null,
string_ids: struct {
...
pub fn err(p: *Parser, tag: Diagnostics.Tag) Compilation.Error!void {
    @setCold(true);
    return p.errExtra(tag, try p.pp.peekToken(), .{ .none = {} });
}
...
fn findLabel(p: *Parser, name: []const u8) ?Token {
...
fn pragma(p: *Parser) Compilation.Error!bool {
    ...
    // while (try p.eatToken(.keyword_pragma)) |_| {
    //     found_pragma = true;
...
pub fn parse(pp: *Preprocessor) Compilation.Error!Tree {
...

    {
        if (p.comp.langopts.hasChar8_T()) {
            try p.syms.defineTypedef(&p, try StrInt.intern(p.comp, "char8_t"), .{ .specifier = .uchar }, Token.invalid, .none);
        }
        try p.syms.defineTypedef(&p, try StrInt.intern(p.comp, "__int128_t"), .{ .specifier = .int128 }, Token.invalid, .none);
        try p.syms.defineTypedef(&p, try StrInt.intern(p.comp, "__uint128_t"), .{ .specifier = .uint128 }, Token.invalid, .none);
        ...
        try p.syms.defineTypedef(&p, try StrInt.intern(p.comp, "__builtin_va_list"), ty.*, Token.invalid, .none);

        if (ty.isArray()) ty.decayArray();

        try p.syms.defineTypedef(&p, try StrInt.intern(p.comp, "__NSConstantString"), pp.comp.types.ns_constant_string.ty, Token.invalid, .none);
    }

    while (try p.eatToken(.eof) == null) {
        if (try p.pragma()) continue;
        if (try p.parseOrNextDecl(staticAssert)) continue;
        if (try p.parseOrNextDecl(decl)) continue;
        if (try p.eatToken(.keyword_extension)) |_| {
        ...
        if (try p.eatToken(.semicolon)) |tok| {
...
fn nextExternDecl(p: *Parser) !void {
    var parens: u32 = 0;
    var tok = try p.pp.peekToken();
    while (true) : (tok = try p.pp.nextToken()) {
        switch (tok.id) {
...
fn decl(p: *Parser) Error!bool {
    ...
    if (decl_spec.noreturn.id != .invalid) {
        const attr = Attribute{ .tag = .noreturn, .args = .{ .noreturn = .{} }, .syntax = .keyword };
        try p.attr_buf.append(p.gpa, .{ .attr = attr, .tok = decl_spec.noreturn });
    }
    ...
    if (init_d.d.func_declarator.id != .invalid and init_d.initializer.node == .none and init_d.d.ty.isFunc()) fn_def: {
        if (decl_spec.auto_type.id != .invalid) {
            try p.errStr(.auto_type_not_allowed, decl_spec.auto_type, "function return type");
            return error.ParsingFailed;
        }
        ...
        switch (tok.id) {
            ...
            .l_brace => {},
            else => if (init_d.d.old_style_func.id == .invalid) {
                try p.errTok(.expected_fn_body, tok);
                return true;
            },
        }
        if (p.func.ty != null) try p.errTok(.func_not_in_root, tok);
        ...
        if (init_d.d.old_style_func.id != .invalid) {
            ...
            param_loop: while (true) {
                ...
                if (try p.eatToken(.semicolon)) |semi| {
                ...
                while (true) {
                    ...
                    if (try p.eatToken(.comma) == null) break;
                }
                _ = try p.expectToken(.semicolon);
            }
        } else {
        ...
        const body = (try p.compoundStmt(true, null)) orelse {
            assert(init_d.d.old_style_func.id != .invalid);
            try p.errTok(.expected_fn_body, try p.pp.peekToken());
            return true;
        };
        p.nodes.set(@intFromEnum(node), .{
            .ty = init_d.d.ty,
            .tag = try decl_spec.validateFnDef(p),
            .data = .{ .decl = .{ .name = try p.addToken(init_d.d.name), .node = body } },
        });
        try p.decl_buf.append(node);
        ...
        if (func.ty == null) {
            ...
            if (p.computed_goto_tok.id != .invalid) {
                if (!p.contains_address_of_label) try p.errTok(.invalid_computed_goto, p.computed_goto_tok);
            }
            p.labels.items.len = 0;
            p.label_count = 0;
            p.contains_address_of_label = false;
            p.computed_goto_tok = Token.invalid;
        }
        return true;
    }
    ...
    while (true) {
        if (init_d.d.old_style_func.id != .invalid) try p.errTok(.invalid_old_style_params, init_d.d.old_style_func);
        ...
        const node = try p.addNode(.{ .ty = init_d.d.ty, .tag = tag, .data = .{
            .decl = .{ .name = try p.addToken(init_d.d.name), .node = init_d.initializer.node },
        } });
        try p.decl_buf.append(node);
            ...
            (p.func.ty != null and decl_spec.storage_class != .@"extern"))
        {
            ...
            try p.syms.defineSymbol(
                ...
                init_d.d.name,
                node,
                if (init_d.d.ty.isConst() or decl_spec.constexpr.id != .invalid) init_d.initializer.val else .{},
                decl_spec.constexpr.id != .invalid,
            );
        } else {
        ...
        if (!warned_auto) {
            if (decl_spec.auto_type.id != .invalid) {
                try p.errTok(.auto_type_requires_single_declarator, decl_spec.auto_type);
                warned_auto = true;
            }
        ...
        init_d = (try p.initDeclarator(&decl_spec, attr_buf_top)) orelse {
            try p.errTok(.expected_ident_or_l_paren, try p.pp.peekToken());
            continue;
        };
    }
...
fn staticAssert(p: *Parser) Error!bool {
    const static_assert = try p.eatToken(.keyword_static_assert) orelse try p.eatToken(.keyword_c23_static_assert) orelse return false;
    const l_paren = try p.expectToken(.l_paren);
    const res_token = try p.pp.peekToken();
    var res = try p.constExpr(.gnu_folding_extension);
    const res_node = res.node;
    const str = if (try p.eatToken(.comma) != null) blk: {
        const tok = try p.pp.peekToken();
        switch (tok.id) {
            .string_literal,
            ...
            => break :blk try p.stringLiteral(),
            else => {
                try p.errTok(.expected_str_literal, tok);
                return error.ParsingFailed;
            },
        }
    } else Result{};
...
pub const DeclSpec = struct {
    ...
    thread_local: Token = Token.invalid,
    constexpr: Token = Token.invalid,
    @"inline": Token = Token.invalid,
    noreturn: Token = Token.invalid,
    auto_type: Token = Token.invalid,
    ty: Type,
...
fn typeof(p: *Parser) Error!?Type {
    var unqual = false;
    switch ((try p.pp.peekToken()).id) {
        .keyword_typeof, .keyword_typeof1, .keyword_typeof2 => p.pp.discardToken(),
        .keyword_typeof_unqual => {
            p.pp.discardToken();
            unqual = true;
        },
...
fn declSpec(p: *Parser) Error!?DeclSpec {
    ...
    const cur = try p.pp.peekToken();
    if (cur.loc.eql(start.loc)) return null;
...
fn attribute(p: *Parser, kind: Attribute.Kind, namespace: ?[]const u8) Error!?TentativeAttribute {
    ...
    switch (p.tok_ids[p.tok_i]) {
        .keyword_const, .keyword_const1, .keyword_const2 => p.pp.discardToken(),
    ...
    const attr = Attribute.fromString(kind, namespace, name) orelse {
        ...
        if (try p.eatToken(.l_paren)) |_| try p.skipTo(.r_paren);
        return null;
    };
    ...
    switch (p.tok_ids[p.tok_i]) {
        ...
        .l_paren => blk: {
            p.pp.discardToken();
            if (try p.eatToken(.r_paren)) |_| break :blk;
            ...
            arg_idx += 1;
            while (try p.eatToken(.r_paren) == null) : (arg_idx += 1) {
...
fn c23AttributeList(p: *Parser) Error!void {
    while (p.tok_ids[p.tok_i] != .r_bracket) {
        ...
        if (try p.eatToken(.colon_colon)) |_| {
...
fn gnuAttribute(p: *Parser) !bool {
    switch (p.tok_ids[p.tok_i]) {
        .keyword_attribute1, .keyword_attribute2 => p.pp.discardToken(),
        else => return false,
    }
...
fn initDeclarator(p: *Parser, decl_spec: *DeclSpec, attr_buf_top: usize) Error!?InitDeclarator {
    ...
    if (try p.eatToken(.equal)) |eq| init: {
        if (decl_spec.storage_class == .typedef or
            (init_d.d.func_declarator.id != .invalid and init_d.d.ty.isFunc()))
        {
...
fn typeSpec(p: *Parser, ty: *Type.Builder) Error!bool {
    ...
    while (true) {
        ...
        switch (tok.id) {
            ...
            .keyword_atomic => {
                ...
                try p.pp.pushCheckpoint();
                p.pp.discardToken();
                const l_paren = try p.eatToken(.l_paren) orelse {
            ...
            => try p.attr_buf.append(p.gpa, .{
                .attr = .{ .tag = .calling_convention, .args = .{
                    .calling_convention = .{ .cc = switch ((try p.pp.peekToken()).id) {
                        .keyword_stdcall,
                        .keyword_stdcall2,
                        => .stdcall,
                        .keyword_thiscall,
            ...
            .identifier, .extended_identifier => {
                ...
                const typedef = (try p.syms.findTypedef(p, interned_name, next_tok, ty.specifier != .none)) orelse break;
    ...
    const current = try p.pp.peekToken();
    return !current.loc.eql(start.loc);
}

fn getAnonymousName(p: *Parser, kind_tok: Token) !StringId {
    const loc = kind_tok.loc;
    const source = p.comp.getSource(loc.id);
    const line_col = source.lineCol(loc);

    const kind_str = switch (kind_tok.id) {
...
fn recordSpec(p: *Parser) Error!Type {
    ...
    const kind_tok = try p.pp.nextToken();
    const is_struct = kind_tok.id == .keyword_struct;
    const attr_buf_top = p.attr_buf.len;
    defer p.attr_buf.len = attr_buf_top;
    try p.attributeSpecifier();

    const maybe_ident = try p.eatIdentifier();
    const l_brace = try p.eatToken(.l_brace) orelse {
        const ident = maybe_ident orelse {
            try p.errTok(.ident_or_l_brace, try p.pp.peekToken());
            return error.ParsingFailed;
        };
        // check if this is a reference to a previous type
        const interned_name = try StrInt.intern(p.comp, p.tokSlice(ident));
        if (try p.syms.findTag(p, interned_name, kind_tok.id, ident, try p.pp.peekToken())) |prev| {
            return prev.ty;
        } else {
            ...
            try p.decl_buf.append(try p.addNode(.{
                ...
                .ty = ty,
                .data = .{ .decl_ref = try p.addToken(ident) },
            }));
            return ty;
        }
    };
    ...
    var done = false;
    errdefer if (!done) p.skipTo(.r_brace) catch unreachable; // TODO: extremely bad
    ...
    const record_ty: *Type.Record = if (maybe_ident) |ident| record_ty: {
        ...
        if (try p.syms.defineTag(p, interned_name, kind_tok.id, ident)) |prev| {
    ...
    if (p.record.flexible_field.id != .invalid) {
        if (p.record_buf.items[record_buf_top..].len == 1 and is_struct) {
            try p.errTok(.flexible_in_empty, p.record.flexible_field);
        }
    }
...
fn recordDeclarator(p: *Parser) Error!bool {
    ...
    while (true) {
        ...
        // invalid means unnamed
        var name_tok: Token = Token.invalid;
        var ty = base_ty;
        if (ty.is(.auto_type)) {
            try p.errStr(.auto_type_not_allowed, try p.pp.peekToken(), if (p.record.kind == .keyword_struct) "struct member" else "union member");
            ty = Type.invalid;
        }
        ...
        if (try p.eatToken(.colon)) |_| bits: {
            const bits_tok = try p.pp.peekToken();
            const res = try p.integerConstExpr(.gnu_folding_extension);
            if (!ty.isInt()) {
            ...
            if (bits_unchecked > bit_size) {
                ...
                break :bits;
            } else if (bits_unchecked == 0 and name_tok.id != .invalid) {
        ...
        if (name_tok.id == .invalid and bits_node == .none) unnamed: {
            ...
            const interned_name = if (name_tok.id != .invalid) try StrInt.intern(p.comp, p.tokSlice(name_tok)) else try p.getAnonymousName(first_tok);
            try p.record_buf.append(.{
                .name = interned_name,
                .ty = ty,
                .name_tok = name_tok,
            ...
            if (name_tok.id != .invalid) try p.record.addField(p, interned_name, name_tok);
            const node = try p.addNode(.{
                .tag = .record_field_decl,
                .ty = ty,
                .data = .{ .decl = .{ .name = try p.addToken(name_tok), .node = bits_node } },
            });
            try p.decl_buf.append(node);
        }
        ...
        if (ty.isFunc()) {
            ...
            if (p.record.flexible_field.id != .invalid) {
                if (p.record.kind == .keyword_struct) {
                    try p.errTok(.flexible_non_final, p.record.flexible_field);
                }
            }
        ...
        } else if (p.record.flexible_field.id != .invalid) {
            if (!p.record.flexible_field.loc.eql(first_tok.loc) and p.record.kind == .keyword_struct) try p.errTok(.flexible_non_final, p.record.flexible_field);
        }
        if (try p.eatToken(.comma) == null) break;
    }

    if (try p.eatToken(.semicolon) == null) {
        const tok = try p.pp.peekToken();
        if (tok.id == .r_brace) {
            try p.errTok(.missing_semicolon, tok);
        } else {
            return p.errExpectedToken(.semicolon, tok);
        }
    }
...
fn enumSpec(p: *Parser) Error!Type {
    const enum_tok = try p.pp.nextToken();
    const attr_buf_top = p.attr_buf.len;
    defer p.attr_buf.len = attr_buf_top;
    try p.attributeSpecifier();

    const maybe_ident = try p.eatIdentifier();
    const fixed_ty = if (try p.eatToken(.colon)) |colon| fixed: {
        try p.pp.pushCheckpoint();
        var pop_checkpoint = true;
        defer if (pop_checkpoint) p.pp.popCheckpoint();
        const fixed = (try p.typeName()) orelse {
            if (p.record.kind != .invalid) {
                // This is a bit field.
                pop_checkpoint = false;
                p.pp.restoreCheckpoint();
                break :fixed null;
            }
            try p.errTok(.expected_type, try p.pp.peekToken());
    ...
    const l_brace = try p.eatToken(.l_brace) orelse {
        ...
        // check if this is a reference to a previous type
        const interned_name = try StrInt.intern(p.comp, p.tokSlice(ident));
        if (try p.syms.findTag(p, interned_name, .keyword_enum, ident, try p.pp.peekToken())) |prev| {
            // only check fixed underlying type in forward declarations and not in references.
            if ((try p.pp.peekToken()).id == .semicolon)
            ...
            const ident_idx = try p.addToken(ident);
            try p.decl_buf.append(try p.addNode(.{
                .tag = .enum_forward_decl,
                .ty = ty,
                .data = .{ .decl_ref = ident_idx },
            }));
            return ty;
        }
    };
    ...
    if (p.enum_buf.items.len == enum_buf_top) try p.errTok(.empty_enum, try p.pp.peekToken());
    try p.expectClosing(l_brace, .r_brace);
    done = true;
    try p.attributeSpecifier();
...
fn checkEnumFixedTy(p: *Parser, fixed_ty: ?Type, ident_tok: Token, prev: Symbol) !void {
    const enum_ty = prev.ty.get(.@"enum").?.data.@"enum";
    if (fixed_ty) |some| {
        if (!enum_ty.fixed) {
...
const Enumerator = struct {
    ...
    fn incr(e: *Enumerator, p: *Parser, tok: Token) !void {
        e.res.node = .none;
    ...
    fn set(e: *Enumerator, p: *Parser, res: Result, tok: Token) !void {
    ...
    fn getTypeSpecifier(e: *const Enumerator, p: *Parser, is_packed: bool, tok: Token) !Type.Specifier {
...
fn enumerator(p: *Parser, e: *Enumerator) Error!?EnumFieldAndNode {
    ...
    const name_tok = (try p.eatIdentifier()) orelse {
        if ((try p.pp.peekToken()).id == .r_brace) return null;
        try p.errTok(.expected_identifier, try p.pp.peekToken());
        try p.skipTo(.r_brace);
        return error.ParsingFailed;
    };
    const attr_buf_top = p.attr_buf.len;
    defer p.attr_buf.len = attr_buf_top;
...
const Declarator = struct {
    name: Token,
    ty: Type,
    func_declarator: Token = Token.invalid,
    old_style_func: Token = Token.invalid,
};
...
fn declarator(
    ...
    if (kind != .abstract and (try p.eatIdentifier()) != null) {
        ...
        return d;
    } else if (try p.eatToken(.l_paren)) |l_paren| blk: {
...
fn directDeclarator(p: *Parser, base_type: Type, d: *Declarator, kind: DeclaratorKind) Error!Type {
    ...
    if (try p.eatToken(.l_bracket)) |l_bracket| {
        ...
        return res_ty;
    } else if (try p.eatToken(.l_paren)) |l_paren| {
        ...
        if (try p.paramDecls(d)) |params| {
            ...
            if (try p.eatToken(.ellipsis)) |_| specifier = .var_args_func;
...
fn pointer(p: *Parser, base_ty: Type) Error!Type {
    var ty = base_ty;
    while (try p.eatToken(.asterisk)) |_| {
        const elem_ty = try p.arena.create(Type);
        elem_ty.* = ty;
        ty = Type{
            .specifier = .pointer,
...
fn typeName(p: *Parser) Error!?Type {
    ...
    if (try p.declarator(ty, .abstract)) |some| {
        if (some.old_style_func.id != .invalid) try p.errTok(.invalid_old_style_params, some.old_style_func);
...
fn initializerItem(p: *Parser, il: *InitList, init_ty: Type) Error!bool {
    ...
    while (true) : (count += 1) {
        errdefer p.skipTo(.r_brace) catch unreachable; // TODO: extremely bad
        ...
        while (true) {
            if (try p.eatToken(.l_bracket)) |l_bracket| {
                ...
                designation = true;
            } else if (try p.eatToken(.period)) |period| {
...
fn asmOperand(p: *Parser, names: *std.ArrayList(Token), constraints: *NodeList, exprs: *NodeList) Error!void {
    if (try p.eatToken(.l_bracket)) |l_bracket| {
        ...
        try names.append(Token.invalid);
    }
...
fn gnuAsmStmt(p: *Parser, quals: Tree.GNUAssemblyQualifiers, l_paren: Token) Error!NodeIndex {
    const asm_str = try p.asmStr();
    try p.checkAsmStr(asm_str.val, l_paren);

    if ((try p.pp.peekToken()).id == .r_paren) {
        return p.addNode(.{
            .tag = .gnu_asm_simple,
            .ty = .{ .specifier = .void },
    ...
    const bytes_needed = expected_items * @sizeOf(Token) + expected_items * 3 * @sizeOf(NodeIndex);
    ...
    var names = std.ArrayList(Token).initCapacity(allocator, expected_items) catch unreachable; // stack allocation already succeeded
    defer names.deinit();
    ...
    if (try p.eatToken(.colon) orelse try p.eatToken(.colon_colon)) |tok| {
        ate_extra_colon = tok.id == .colon_colon;
        if (!ate_extra_colon) {
            const next_tok = try p.pp.peekToken();
            if (next_tok.id.isStringLiteral() or next_tok.id == .l_bracket) {
                while (true) {
    ...
    // Inputs
    if (ate_extra_colon or (try p.pp.peekToken()).id == .colon or (try p.pp.peekToken()).id == .colon_colon) {
        if (ate_extra_colon) {
            ate_extra_colon = false;
        } else {
            ate_extra_colon = (try p.pp.nextToken()).id == .colon_colon;
        }
        if (!ate_extra_colon) {
            if ((try p.pp.peekToken()).id.isStringLiteral() or (try p.pp.peekToken()).id == .l_bracket) {
                while (true) {
                    try p.asmOperand(&names, &constraints, &exprs);
                    if (try p.eatToken(.comma) == null) break;
                }
    ...
    if (ate_extra_colon or (try p.pp.peekToken()).id == .colon or (try p.pp.peekToken()).id == .colon_colon) {
        ...
        if (!ate_extra_colon and (try p.pp.peekToken()).id.isStringLiteral()) {
            while (true) {
    ...
    if (!quals.goto and ((try p.pp.peekToken()).id != .r_paren or ate_extra_colon)) {
        const cur = try p.pp.peekToken();
        try p.errExtra(.expected_token, cur, .{ .tok_id = .{ .actual = cur.id, .expected = .r_paren } });
        return error.ParsingFailed;
    }

    // Goto labels
    var num_labels: u32 = 0;
    if (ate_extra_colon or (try p.pp.peekToken()).id == .colon) {
        if (!ate_extra_colon) {
            p.pp.discardToken();
        }
        while (true) {
            ...
            const label_addr_node = try p.addNode(.{
                .tag = .addr_of_label,
                .data = .{ .decl_ref = try p.addToken(label) },
                .ty = result_ty,
            });
        ...
        const tok = try p.pp.peekToken();
        try p.errExtra(.expected_token, tok, .{ .tok_id = .{ .actual = tok.id, .expected = .colon } });
        return error.ParsingFailed;
    }
...
fn checkAsmStr(p: *Parser, asm_str: Value, tok: Token) !void {
...
fn assembly(p: *Parser, kind: enum { global, decl_label, stmt }) Error!?NodeIndex {
    const asm_tok = try p.pp.peekToken();
    switch (asm_tok.id) {
        .keyword_asm => {
            try p.errTok(.extension_token_used, asm_tok);
            p.pp.discardToken();
        },
        .keyword_asm1, .keyword_asm2 => p.pp.discardToken(),
        else => return null,
    }

    var tok = try p.pp.peekToken();
    if (!tok.id.canOpenGCCAsmStmt()) {
        return p.msvcAsmStmt();
    }

    var quals: Tree.GNUAssemblyQualifiers = .{};
    while (true) : (tok = try p.pp.nextToken()) switch (tok.id) {
        .keyword_volatile, .keyword_volatile1, .keyword_volatile2 => {
            if (kind != .stmt) try p.errStr(.meaningless_asm_qual, tok, "volatile");
            if (quals.@"volatile") try p.errStr(.duplicate_asm_qual, tok, "volatile");
            quals.@"volatile" = true;
        },
        .keyword_inline, .keyword_inline1, .keyword_inline2 => {
            if (kind != .stmt) try p.errStr(.meaningless_asm_qual, tok, "inline");
            if (quals.@"inline") try p.errStr(.duplicate_asm_qual, tok, "inline");
            quals.@"inline" = true;
        },
        .keyword_goto => {
            if (kind != .stmt) try p.errStr(.meaningless_asm_qual, tok, "goto");
            if (quals.goto) try p.errStr(.duplicate_asm_qual, tok, "goto");
            quals.goto = true;
        },
        else => break,
    };
    ...
    const asm_tok_idx = try p.addToken(asm_tok);
    switch (kind) {
        .decl_label => {
            const asm_str = try p.asmStr();
        ...
        .global => {
            ...
            result_node = try p.addNode(.{
                .tag = .file_scope_asm,
                .ty = .{ .specifier = .void },
                .data = .{ .decl = .{ .name = asm_tok_idx, .node = asm_str.node } },
            });
        },
...
fn asmStr(p: *Parser) Error!Result {
    ...
    while (true) : (tok = try p.pp.nextToken()) switch (tok.id) {
        .string_literal, .unterminated_string_literal => literal_found = true,
        .string_literal_utf_16, .string_literal_utf_8, .string_literal_utf_32 => {
            try p.errStr(.invalid_asm_str, tok, "unicode");
            return error.ParsingFailed;
        },
        .string_literal_wide => {
            try p.errStr(.invalid_asm_str, tok, "wide");
            return error.ParsingFailed;
        },
        else => {
            if (!literal_found) {
                try p.errStr(.expected_str_literal_in, tok, "asm");
                return error.ParsingFailed;
            }
            break;
        },
...
fn stmt(p: *Parser) Error!NodeIndex {
    ...
    if (try p.eatToken(.keyword_if)) |_| {
        const l_paren = try p.expectToken(.l_paren);
        ...
        const @"else" = if (try p.eatToken(.keyword_else)) |_| try p.stmt() else .none;
    ...
    if (try p.eatToken(.keyword_switch)) |_| {
        const l_paren = try p.expectToken(.l_paren);
        const cond_tok = p.tok_i;
        var cond = try p.expr();
        try cond.expect(p);
    ...
    if (try p.eatToken(.keyword_while)) |_| {
        const l_paren = try p.expectToken(.l_paren);
        const cond_tok = p.tok_i;
        var cond = try p.expr();
        try cond.expect(p);
    ...
    if (try p.eatToken(.keyword_do)) |_| {
        const body = body: {
            const old_loop = p.in_loop;
            p.in_loop = true;
            defer p.in_loop = old_loop;
    ...
    if (try p.eatToken(.keyword_for)) |_| {
        try p.syms.pushScope(p);
        defer p.syms.popScope();
        const decl_buf_top = p.decl_buf.items.len;
    ...
    if (try p.eatToken(.keyword_goto)) |goto_tok| {
        if (try p.eatToken(.asterisk)) |_| {
            const expr_tok = p.tok_i;
            var e = try p.expr();
            try e.expect(p);
...
fn labeledStmt(p: *Parser) Error!?NodeIndex {
    ...
    } else if (try p.eatToken(.keyword_case)) |case| {
        const first_item = try p.integerConstExpr(.gnu_folding_extension);
        const ellipsis = p.tok_i;
        const second_item = if (try p.eatToken(.ellipsis) != null) blk: {
    ...
    } else if (try p.eatToken(.keyword_default)) |default| {
        _ = try p.expectToken(.colon);
        const s = try p.labelableStmt();
        const node = try p.addNode(.{
            .tag = .default_stmt,
...
fn compoundStmt(p: *Parser, is_fn_body: bool, stmt_expr_state: ?*StmtExprState) Error!?NodeIndex {
    const l_brace = try p.eatToken(.l_brace) orelse return null;
    ...
    while (try p.eatToken(.r_brace) == null) : (_ = try p.pragma()) {
        ...
        if (try p.eatToken(.keyword_extension)) |ext| {
            const saved_extension = p.extension_suppressed;
            defer p.extension_suppressed = saved_extension;
            p.extension_suppressed = true;
...
fn parseOrNextStmt(p: *Parser, comptime func: fn (*Parser) Error!bool, l_brace: Token) !bool {
    return func(p) catch |er| switch (er) {
        error.ParsingFailed => {
            try p.nextStmt(l_brace);
            return true;
...
fn nextStmt(p: *Parser, l_brace: Token) !void {
    var parens: u32 = 0;
    var tok = try p.pp.peekToken();
    while (tok.id != .eof) : (tok = try p.pp.nextToken()) {
        switch (tok.id) {
            .l_paren, .l_brace, .l_bracket => parens += 1,
...
pub fn macroExpr(p: *Parser) Compilation.Error!bool {
    ...
    if (res.val.opt_ref == .none) {
        try p.errTok(.expected_expr, try p.pp.peekToken());
        return false;
    }
...
pub const Result = struct {
    ...
    fn maybeWarnUnused(res: Result, p: *Parser, expr_start: Token, err_start: usize) Error!void {
    ...
    fn adjustCondExprPtrs(a: *Result, tok: Token, b: *Result, p: *Parser) !bool {
    ...
    fn adjustTypes(a: *Result, tok: Token, b: *Result, p: *Parser, kind: enum {
        integer,
        arithmetic,
        boolean_logic,
        relational,
    ...
    fn boolCast(res: *Result, p: *Parser, bool_ty: Type, tok: Token) Error!void {
        if (res.ty.isArray()) {
    ...
    fn intCast(res: *Result, p: *Parser, int_ty: Type, tok: Token) Error!void {
    ...
    fn floatToIntWarning(res: *Result, p: *Parser, int_ty: Type, old_value: Value, change_kind: Value.FloatToIntChangeKind, tok: Token) !void {
        switch (change_kind) {
    ...
    fn usualUnaryConversion(res: *Result, p: *Parser, tok: Token) Error!void {
    ...
    fn usualArithmeticConversion(a: *Result, b: *Result, p: *Parser, tok: Token) Error!void {
    ...
    fn invalidBinTy(a: *Result, tok: Token, b: *Result, p: *Parser) Error!bool {
    ...
    fn castType(res: *Result, p: *Parser, to: Type, operand_tok: Token, l_paren: Token) !void {
...
fn expr(p: *Parser) Error!Result {
    var expr_start = try p.pp.peekToken();
    var err_start = p.comp.diagnostics.list.items.len;
    var lhs = try p.assignExpr();
    const maybe_comma = try p.pp.peekToken();
    if (maybe_comma.id == .comma) try lhs.expect(p);
    while (try p.eatToken(.comma)) |_| {
        try lhs.maybeWarnUnused(p, expr_start, err_start);
        expr_start = try p.pp.peekToken();
        err_start = p.comp.diagnostics.list.items.len;
...
/// TODO: move to tree?
fn tokToTag(tok: Token) Tree.Tag {
...
fn assignExpr(p: *Parser) Error!Result {
    ...
    const tok = try p.pp.peekToken();
    const eq = try p.eatToken(.equal);
    const mul = eq orelse try p.eatToken(.asterisk_equal);
    const div = mul orelse try p.eatToken(.slash_equal);
    const mod = div orelse try p.eatToken(.percent_equal);
    const add = mod orelse try p.eatToken(.plus_equal);
    const sub = add orelse try p.eatToken(.minus_equal);
    const shl = sub orelse try p.eatToken(.angle_bracket_angle_bracket_left_equal);
    const shr = shl orelse try p.eatToken(.angle_bracket_angle_bracket_right_equal);
    const bit_and = shr orelse try p.eatToken(.ampersand_equal);
    const bit_xor = bit_and orelse try p.eatToken(.caret_equal);
    const bit_or = bit_xor orelse try p.eatToken(.pipe_equal);

    const tag = tokToTag(bit_or orelse return lhs);
    var rhs = try p.assignExpr();
    try rhs.expect(p);
    try rhs.lvalConversion(p);
...
fn condExpr(p: *Parser) Error!Result {
    ...
    if (cond.empty(p) or (try p.eatToken(.question_mark)) == null) return cond;
    try cond.lvalConversion(p);
    ...
    const maybe_colon = try p.eatToken(.colon);
...
fn lorExpr(p: *Parser) Error!Result {
    ...
    while (try p.eatToken(.pipe_pipe)) |tok| {
...
fn landExpr(p: *Parser) Error!Result {
    ...
    while (try p.eatToken(.ampersand_ampersand)) |tok| {
        if (lhs.val.opt_ref != .none and !lhs.val.toBool(p.comp)) p.no_eval = true;
...
fn xorExpr(p: *Parser) Error!Result {
    ...
    if (lhs.empty(p)) return lhs;
    while (try p.eatToken(.caret)) |tok| {
...
fn andExpr(p: *Parser) Error!Result {
    ...
    if (lhs.empty(p)) return lhs;
    while (try p.eatToken(.ampersand)) |tok| {
...
fn eqExpr(p: *Parser) Error!Result {
    ...
    if (lhs.empty(p)) return lhs;
    while (true) {
        const eq = try p.eatToken(.equal_equal);
        const ne = eq orelse try p.eatToken(.bang_equal);
        const tag = tokToTag(ne orelse break);
...
fn compExpr(p: *Parser) Error!Result {
    ...
    if (lhs.empty(p)) return lhs;
    while (true) {
        const lt = try p.eatToken(.angle_bracket_left);
        const le = lt orelse try p.eatToken(.angle_bracket_left_equal);
        const gt = le orelse try p.eatToken(.angle_bracket_right);
        const ge = gt orelse try p.eatToken(.angle_bracket_right_equal);
        const tag = tokToTag(ge orelse break);
...
fn shiftExpr(p: *Parser) Error!Result {
    ...
    if (lhs.empty(p)) return lhs;
    while (true) {
        const shl = try p.eatToken(.angle_bracket_angle_bracket_left);
        const shr = shl orelse try p.eatToken(.angle_bracket_angle_bracket_right);
        const tag = tokToTag(shr orelse break);
...
fn addExpr(p: *Parser) Error!Result {
    ...
    if (lhs.empty(p)) return lhs;
    while (true) {
        const plus = try p.eatToken(.plus);
        const minus = plus orelse try p.eatToken(.minus);
        const tag = tokToTag(minus orelse break);
...
fn mulExpr(p: *Parser) Error!Result {
    ...
    if (lhs.empty(p)) return lhs;
    while (true) {
        const mul = try p.eatToken(.asterisk);
        const div = mul orelse try p.eatToken(.slash);
        const percent = div orelse try p.eatToken(.percent);
        const tag = tokToTag(percent orelse break);
...
fn castExpr(p: *Parser) Error!Result {
    try p.pp.pushCheckpoint();
    if (try p.eatToken(.l_paren)) |l_paren| cast_expr: {
        const tok = try p.pp.peekToken();
        if (tok.id == .l_brace) {
        ...
        if ((try p.pp.peekToken()).id == .l_brace) {
...
fn builtinVaArg(p: *Parser) Error!Result {
    const builtin_tok = try p.pp.nextToken();
...
fn offsetofMemberDesignator(p: *Parser, base_ty: Type, want_bits: bool) Error!Result {
    errdefer p.skipTo(.r_paren) catch unreachable; // TODO: extremely bad
    ...
    while (true) switch (p.tok_ids[p.tok_i]) {
        ...
        .l_bracket => {
            const l_bracket_tok = try p.pp.nextToken();
...
fn unExpr(p: *Parser) Error!Result {
    ...
    switch (tok.id) {
        .ampersand_ampersand => {
            const address_tok = try p.pp.nextToken();
            const name_tok = try p.expectIdentifier();
            ...
            return Result{
                .node = try p.addNode(.{
                    .tag = .addr_of_label,
                    .data = .{ .decl_ref = try p.addToken(name_tok) },
                    .ty = result_ty,
                }),
                .ty = result_ty,
            };
        ...
        .asterisk => {
            const asterisk_loc = try p.pp.nextToken();
            var operand = try p.castExpr();
            try operand.expect(p);
        ...
        .plus_plus => {
            ...
            if (operand.ty.isComplex())
                try p.errStr(.complex_prefix_postfix_op, try p.pp.peekToken(), try p.typeStr(operand.ty));
        ...
        .minus_minus => {
            ...
            if (!operand.ty.isScalar())
                try p.errStr(.invalid_argument_un, tok, try p.typeStr(operand.ty));
            if (operand.ty.isComplex())
                try p.errStr(.complex_prefix_postfix_op, tok, try p.typeStr(operand.ty));
        ...
        .keyword_sizeof => {
            p.pp.discardToken();
            try p.pp.pushCheckpoint();
            const expected_paren = try p.pp.peekToken();
            var res = Result{};
            if (try p.typeName()) |ty| {
                p.pp.popCheckpoint();
                res.ty = ty;
            ...
            if (res.ty.sizeof(p.comp)) |size| {
                ...
                if (res.ty.hasIncompleteSize()) {
                    try p.errStr(.invalid_sizeof, tok, try p.typeStr(res.ty));
                    res.ty = Type.invalid;
                } else {
                    res.ty = p.comp.types.size;
                }
        ...
        .keyword_imag1, .keyword_imag2 => {
            const imag_tok = try p.pp.nextToken();
        ...
        .keyword_real1, .keyword_real2 => {
            const real_tok = try p.pp.nextToken();
...
fn suffixExpr(p: *Parser, lhs: Result) Error!Result {
    ...
    switch (tok.id) {
        ...
        .l_bracket => {
            const l_bracket = try p.pp.nextToken();
            var index = try p.expr();
            try index.expect(p);
            try p.expectClosing(l_bracket, .r_bracket);
...
fn callExpr(p: *Parser, lhs: Result) Error!Result {
    const l_paren = try p.pp.nextToken();
    ...
    while (try p.eatToken(.r_paren) == null) {
...
fn primaryExpr(p: *Parser) Error!Result {
    if (try p.eatToken(.l_paren)) |l_paren| {
        var e = try p.expr();
        try e.expect(p);
        try p.expectClosing(l_paren, .r_paren);
    ...
    switch (tok.id) {
        ...
        .keyword_nullptr => {
            p.pp.discardToken();
            try p.errStr(.pre_c23_compat, tok, "'nullptr'");
            return Result{
                .val = Value.null,
        ...
        .macro_func, .macro_function => {
            ...
            var tok_idx = try p.addToken(tok);
            if (p.func.ident) |some| {
                ty = some.ty;
                tok_idx = p.nodes.items(.data)[@intFromEnum(some.node)].decl.name;
            } else if (p.func.ty) |_| {
                const strings_top = p.strings.items.len;
                defer p.strings.items.len = strings_top;
        ...
        .macro_pretty_func => {
            ...
            return Result{
                ...
                .node = try p.addNode(.{
                    .tag = .decl_ref_expr,
                    .ty = ty,
                    .data = .{ .decl_ref = try p.addNode(tok) },
                }),
            };
        },
        .string_literal,
        ...
        .embed_byte => {
            assert(!p.in_macro);
            const loc = (try p.pp.nextToken()).loc;
...
fn stringLiteral(p: *Parser) Error!Result {
    var string_end = try p.pp.peekToken();
    var string_kind: text_literal.Kind = .char;
    while (text_literal.Kind.classify(string_end.id, .string_literal)) |next| : (string_end = try p.pp.nextToken()) {
        string_kind = string_kind.concat(next) catch {
            try p.errTok(.unsupported_str_cat, string_end);
            while ((try p.pp.peekToken()).id.isStringLiteral()) : (p.pp.discardToken()) {}
            return error.ParsingFailed;
        };
...
fn charLiteral(p: *Parser) Error!Result {
    const tok = try p.pp.nextToken();
    const char_kind = text_literal.Kind.classify(tok.id, .char_literal) orelse {
        if (tok.id == .empty_char_literal) {
            try p.errTok(.empty_char_literal_error, tok);
        } else if (tok.id == .unterminated_char_literal) {
            try p.errTok(.unterminated_char_literal_error, tok);
        } else unreachable;
        return .{
            .ty = Type.int,
            .val = Value.zero,
    ...
    if (char_kind == .utf_8) try p.errTok(.u8_char_lit, tok);
    var val: u32 = 0;

    const slice = char_kind.contentSlice(p.tokSlice(tok));
    ...
    if (slice.len == 1 and std.ascii.isASCII(slice[0])) {
        ...
        for (char_literal_parser.errors()) |item| {
            try p.errExtra(item.tag, tok, item.extra);
        }
    }
...
fn getIntegerPart(p: *Parser, buf: []const u8, prefix: NumberPrefix, tok: Token) ![]const u8 {
    if (buf[0] == '.') return "";

    if (!prefix.digitAllowed(buf[0])) {
        switch (prefix) {
            .binary => try p.errExtra(.invalid_binary_digit, tok, .{ .ascii = @intCast(buf[0]) }),
            .octal => try p.errExtra(.invalid_octal_digit, tok, .{ .ascii = @intCast(buf[0]) }),
            .hex => try p.errStr(.invalid_int_suffix, tok, buf),
            .decimal => unreachable,
        }
        return error.ParsingFailed;
    }
    ...
    for (buf, 0..) |c, idx| {
        ...
        switch (c) {
            ...
            'p', 'P' => return if (prefix == .hex) buf[0..idx] else {
                try p.errStr(.invalid_int_suffix, tok, buf[idx..]);
                return error.ParsingFailed;
            },
            'e', 'E' => {
                switch (prefix) {
                    ...
                    .binary => try p.errExtra(.invalid_binary_digit, tok, .{ .ascii = @intCast(c) }),
                    .octal => try p.errExtra(.invalid_octal_digit, tok, .{ .ascii = @intCast(c) }),
                }
                return error.ParsingFailed;
            },
            '0'...'9', 'a'...'d', 'A'...'D', 'f', 'F' => {
                if (!prefix.digitAllowed(c)) {
                    switch (prefix) {
                        .binary => try p.errExtra(.invalid_binary_digit, tok, .{ .ascii = @intCast(c) }),
                        .octal => try p.errExtra(.invalid_octal_digit, tok, .{ .ascii = @intCast(c) }),
                        .decimal, .hex => try p.errStr(.invalid_int_suffix, tok, buf[idx..]),
                    }
                    return error.ParsingFailed;
                }
            },
...
fn fixedSizeInt(p: *Parser, base: u8, buf: []const u8, suffix: NumberSuffix, tok: Token) !Result {
    var val: u64 = 0;
    var overflow = false;
    for (buf) |c| {
    ...
    if (overflow) {
        try p.errTok(.int_literal_too_big, tok);
    ...
    if (suffix.isSignedInteger()) {
        if (val > p.comp.types.intmax.maxInt(p.comp)) {
            try p.errTok(.implicitly_unsigned_literal, tok);
        }
    }
...
fn parseInt(p: *Parser, prefix: NumberPrefix, buf: []const u8, suffix: NumberSuffix, tok: Token) !Result {
    if (prefix == .binary) {
        try p.errTok(.binary_integer_literal, tok);
    }
    const base = @intFromEnum(prefix);
    var res = if (suffix.isBitInt())
        try p.bitInt(base, buf, suffix, tok)
    else
        try p.fixedSizeInt(base, buf, suffix, tok);

    if (suffix.isImaginary()) {
        try p.errTok(.gnu_imaginary_constant, tok);
...
fn bitInt(p: *Parser, base: u8, buf: []const u8, suffix: NumberSuffix, tok: Token) Error!Result {
    try p.errStr(.pre_c23_compat, tok, "'_BitInt' suffix for literals");
    try p.errTok(.bitint_suffix, tok);
    ...
    const bits_needed: std.math.IntFittingRange(0, Compilation.bit_int_max_bits) = blk: {
        ...
        if (bits_needed > Compilation.bit_int_max_bits) {
            ...
            try p.errStr(.bit_int_too_big, tok, specifier.str(p.comp.langopts).?);
            return error.ParsingFailed;
        }
...
fn getFracPart(p: *Parser, buf: []const u8, prefix: NumberPrefix, tok: Token) ![]const u8 {
    if (buf.len == 0 or buf[0] != '.') return "";
    assert(prefix != .octal);
    if (prefix == .binary) {
        try p.errStr(.invalid_int_suffix, tok, buf);
        return error.ParsingFailed;
    }
    for (buf, 0..) |c, idx| {
        if (idx == 0) continue;
...
fn getExponent(p: *Parser, buf: []const u8, prefix: NumberPrefix, tok: Token) ![]const u8 {
    if (buf.len == 0) return "";

    switch (buf[0]) {
        'e', 'E' => assert(prefix == .decimal),
        'p', 'P' => if (prefix != .hex) {
            try p.errStr(.invalid_float_suffix, tok, buf);
            return error.ParsingFailed;
        },
        else => return "",
    }
    ...
    if (std.mem.indexOfAny(u8, exponent, "0123456789") == null) {
        try p.errTok(.exponent_has_no_digits, tok);
        return error.ParsingFailed;
    }
    return exponent;
}

/// TOKEN TODO review this doc comment
/// Using an explicit `tok_i` parameter instead of `p.tok_i` makes it easier
/// to parse numbers in pragma handlers.
pub fn parseNumberToken(p: *Parser, tok: Token) !Result {
    const buf = p.tokSlice(tok);
    const prefix = NumberPrefix.fromString(buf);
    const after_prefix = buf[prefix.stringLen()..];

    const int_part = try p.getIntegerPart(after_prefix, prefix, tok);

    const after_int = after_prefix[int_part.len..];

    const frac = try p.getFracPart(after_int, prefix, tok);
    const after_frac = after_int[frac.len..];

    const exponent = try p.getExponent(after_frac, prefix, tok);
    ...
    const suffix = NumberSuffix.fromString(suffix_str, if (is_float) .float else .int) orelse {
        if (is_float) {
            try p.errStr(.invalid_float_suffix, tok, suffix_str);
        } else {
            try p.errStr(.invalid_int_suffix, tok, suffix_str);
        }
        return error.ParsingFailed;
    };
    ...
    if (is_float) {
        ...
        if (prefix == .hex and exponent.len == 0) {
            try p.errTok(.hex_floating_constant_requires_exponent, tok);
            return error.ParsingFailed;
        }
        const number = buf[0 .. buf.len - suffix_str.len];
        return p.parseFloat(number, suffix);
    } else {
        return p.parseInt(prefix, int_part, suffix, tok);
    }
}

fn ppNum(p: *Parser) Error!Result {
    const tok = try p.pp.nextToken();
    var res = try p.parseNumberToken(tok);
    if (p.in_macro) {
        if (res.ty.isFloat() or !res.ty.isReal()) {
            try p.errTok(.float_literal_in_pp_expr, tok);
            return error.ParsingFailed;
        }
...
fn genericSelection(p: *Parser) Error!Result {
    ...
    while (true) {
        ...
        } else if (try p.eatToken(.keyword_default)) |tok| {
            if (default_tok) |prev| {

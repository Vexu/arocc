    ...
    while (true) : (count += 1) {
        ...
        if (is_str_init and p.isStringInit(init_qt)) {
            ...
            if (designation) {
                ...
                defer tmp_il.deinit(p.gpa);
                saw = try p.initializerItem(&tmp_il, .void);
                if (!warned_excess) try p.errTok(if (init_qt.is(p.comp, .array)) .excess_array_init else .excess_struct_init, first_tok);
                warned_excess = true;
            }
        } else single_item: {
            first_tok = p.tok_i;
            ...
            if (p.getNode(node_idx, tag)) |node| {
                switch (tag) {
                    ...
                    .decl_ref_expr => {
                        ...
                        try p.errStr(.assign_to_const_var, tok, try p.assignToConstVarStr(var_name, node.qt));
                    ...
                    else => try p.errTok(.not_assignable, tok),
                }
            };
    }
}

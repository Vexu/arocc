...
fn addInst(irb: *IrBuilder, tag: Ir.Inst.Tag, data: Ir.Inst.Data, ty: Type) !Ir.Ref {
    ...
    if (tag == .alloc) {
        try irb.body.insert(irb.comp.gpa, irb.last_alloc_index, ref);
        irb.last_alloc_index = 0;
    } else {
...
fn genFn(irb: *IrBuilder, decl: NodeIndex) Error!void {
    ...
    for (func_ty.data.func.params) |param, i| {
        const alloc = @intToEnum(Ir.Ref, irb.instructions.len);
        try irb.instructions.append(
            irb.comp.gpa,
            .{ .tag = .arg, .data = .{ .arg = @intCast(u32, i) }, .ty = try irb.genType(param.ty) },
        );
        const mapper = irb.comp.string_interner.getSlowTypeMapper();
        try irb.symbols.append(irb.comp.gpa, .{ .name = mapper.lookup(param.name), .val = alloc });
    }
...
fn genNode(irb: *IrBuilder, node: NodeIndex) Error!Ir.Ref {
    ...
    switch (irb.node_tag[@enumToInt(node)]) {
        ...
        .@"var" => {
            const size = ty.sizeof(irb.comp) orelse return error.IrGenFailed; // TODO vla
            const alloc = try irb.addInst(.alloc, .{ .alloc = size }, .{ .specifier = .pointer });
            try irb.symbols.append(irb.comp.gpa, .{ .name = irb.tree.tokSlice(data.decl.name), .val = alloc });
            if (data.decl.node != .none) {
                const res = try irb.genNode(data.decl.node);
                _ = try irb.addInst(.store, .{ .bin = .{ .lhs = alloc, .rhs = res } }, ty);
            }
            return alloc;
        },
        .labeled_stmt => {
            _ = try irb.addInst(.label, .{ .label = "label" }, ty);
        ...
        //     const jmp_else_index = irb.instructions.len;
        ...
        //     _ = try irb.addInst(.jmp, undefined, ty);

        //     const else_label = try irb.addInst(.label, .{ .none = {} }, ty);
        //     _ = try irb.genNode(irb.tree.data[data.if3.body + 1]); // else

        //     const end_label = try irb.addInst(.label, .{ .none = {} }, ty);

        //     irb.instructions.items(.data)[jmp_else_index] = .{
        ...
        //     const end_label = try irb.addInst(.label, .{ .label = "end" }, ty);
        //     irb.instructions.items(.data)[jmp_index] = .{
        ...
        // },
        .default_stmt => {
            irb.wip_switch.default = try irb.addInst(.label, .{ .label = "default" }, ty);
        ...
        .return_stmt => {
            // if (data.un == .none)
            //     return irb.addInst(.ret_void, .{ .none = {} }, ty);

            const operand = try irb.genNode(data.un);
            return irb.addInst(.ret, .{ .un = operand }, ty);
        },
        .comma_expr => {
        ...
        .assign_expr => {
            ...
            _ = try irb.addInst(.store, .{ .bin = .{ .lhs = lhs, .rhs = rhs } }, ty);
            return rhs;
        },
        ...
        .decl_ref_expr => {
            const name = irb.tree.tokSlice(data.decl_ref);
            var i = irb.symbols.items.len;
            while (i > 0) {
                i -= 1;
                if (std.mem.eql(u8, irb.symbols.items[i].name, name)) {
...
fn genCompoundAssign(irb: *IrBuilder, node: NodeIndex, tag: Ir.Inst.Tag) Error!Ir.Ref {
    ...
    _ = try irb.addInst(.store, .{ .bin = .{ .lhs = lhs, .rhs = res } }, .{ .specifier = .void });
    return res;
}

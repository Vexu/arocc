...
// until record layout and _bifoffset of are merged. skip all asserts in the tests
// This will just test the parser.
fn whichTests(comptime skip_all: bool) []const TestControl {
    if (skip_all) {
        return &[_]TestControl{
            .{
        ...
        return &[_]TestControl{
            ...
            .{
                .target = "aarch64-generic-macos-none:Clang",
                .non_working_tests = &.{
                    "0011", "0014", "0044", "0046",
                },
                .skip_offset_checks = &.{},
...
// a zig #ifdef
const working_targets = whichTests(true);
...
pub fn main() !void {
    ...
    // for each targets, run all the tests

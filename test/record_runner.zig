...
// used a fixed size allocator for each test file
// and every compiler instance.
var fixie: std.mem.Allocator = undefined;

// these tests don't work for any platfrom due to arocc bugs.
// skip entirely.
...
const do_single: ?Single = if (false) one_test else null;
...
pub fn main() !void {
    ...
    // max file sizse < 2M. max total use < 12M
    ...
    for (cases.items) |path| {
        ...
        for (test_targets.items) |test_case| {
            if (do_single) |single| {
                if (std.ascii.indexOfIgnoreCase(test_case.target, single.target) == null) continue;
                if (std.mem.indexOf(u8, path, single.c_test) == null) continue;
            }
            fixed_alloc.end_index = mem_reset_pos;

            try singleRun(path, source, test_case, &stats);
...
fn singleRun(path: []const u8, source: []const u8, test_case: TestCase, state: *Stats) !void {
    // prepare compiler
    //
    ...
    // zig panics when trying to setup
    // some targets. Just skip them.
    const target = setTarget(&comp, test_case.target) catch {
        return;
    };
    // targets arocc panic on.
...
/// get zig std.Target from string. string format arch-cpu-os-abi
fn getTarget(zig_target_string: []const u8) !std.Target {
    ...
    // zig has some "unreachable" code in defaultVersionRange. Try to trap it here
    // and return an error.
    if (tag == .macos) {
        switch (ret.cpu.arch) {
            .x86_64,
            .aarch64,
            => {},
            else => return TargetError.InvalidTarget,
        }
    }
...
fn parseTargetsFromCode(source: []const u8) !std.ArrayList(TestCase) {
    var result = std.ArrayList(TestCase).init(fixie);

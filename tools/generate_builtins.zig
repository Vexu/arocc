...
const generated_file_path = "bf/Builtins.def";
...
pub fn main() !void {
    ...
    while (args_it.next()) |def_path| {
        const def_contents = try std.fs.cwd().readFileAlloc(def_path, allocator, .unlimited);
        ...
        while (line_it.next()) |line| {
            ...
            switch (macro) {
                .TARGET_BUILTIN, .TARGET_HEADER_BUILTIN => {},
                .CUSTOM_BUILTIN, .UNALIASED_CUSTOM_BUILTIN => continue, // TODO
                else => continue,
            }
            ...
            // std.debug.print("\r{s} {s}", .{name, params_without_quotes});
            ...
            switch (macro) {
                ...
                .TARGET_BUILTIN => {
                    const feature = param_it.next() orelse continue;
                    if (feature.len > 2) {
                        // std.debug.print("feature '{s}'\n", .{feature});
                        builtin_data.properties.feature = try arena.dupe(u8, feature);
                    }
                },
                .TARGET_HEADER_BUILTIN => {
                    const header = param_it.next() orelse continue;
                    builtin_data.properties.header = header_map.get(header) orelse {
                        std.debug.print("{s}: unknown header '{s}', skipping\n", .{ name, header });
                        continue;
            ...
            if (builtin_data.properties.feature) |some| {
                std.mem.replaceScalar(u8, some, '-', '_');
                std.mem.replaceScalar(u8, some, '.', '_');
            }
...
fn writeData(writer: anytype, data_array: []const *BuiltinData, names_array: []const []const u8) !void {
    for (data_array, names_array, 0..) |data, name, i| {
        if (data.properties.feature != null and data.properties.target_set.count() > 1) {
        ...
        if (data.properties.feature) |feature| {
            var set_it = data.properties.target_set.iterator();
            const target = set_it.next().?;
            if (set_it.next()) |some| {
                std.debug.print("TODO extra target {s} {s}, {s}\n", .{ name, @tagName(target), @tagName(some) });
            }
            try writer.print("    .features = {s}\n", .{feature});
        }
        if (!allDefaultAttributes(data.properties.attributes)) {
            ...
            inline for (@typeInfo(@TypeOf(data.properties.attributes)).@"struct".fields) |field| {
                const default = @as(*align(1) const field.type, @ptrCast(field.default_value_ptr.?)).*;
                if (@field(data.properties.attributes, field.name) != default) {
                    if (!first) try writer.writeAll(", ");
                    try writer.print(".{f} = ", .{std.zig.fmtId(field.name)});
                    switch (@typeInfo(field.type)) {
                        .@"enum" => {
                            try writer.print(".{f}", .{std.zig.fmtId(@tagName(@field(data.properties.attributes, field.name)))});
                        },
                        .bool, .int => {
...
const header_map = std.StaticStringMap(Properties.Header).initComptime(.{
    .{ "NO_HEADER", .none },
    .{ "STDIO_H", .stdio },
    .{ "STDLIB_H", .stdlib },
    .{ "SETJMPEX_H", .setjmpex },
    ...
    // .{ "UTILITY", .utility },
    .{ "INTRIN_H", .intrin },
    .{ "IMMINTRIN_H", .immintrin },
    .{ "XMMINTRIN_H", .xmmintrin },
    .{ "EMMINTRIN_H", .emmintrin },
    .{ "MMINTRIN_H", .mmintrin },
    .{ "ARMACLE_H", .arm_acle },
});

const allowed_languages_map = std.StaticStringMap(Properties.Language).initComptime(.{
...
const targets_map = std.StaticStringMapWithEql(Properties.Target, std.static_string_map.eqlAsciiIgnoreCase).initComptime(.{
    .{ "aarch64", .aarch64 },
    ...
    .{ "x86_64", .x86_64 },
    .{ "xcore", .xcore },
    .{ "loongarchbase", .loong_arch },
    .{ "loongarchlasx", .loong_arch },
    .{ "loongarchlsx", .loong_arch },
    .{ "spirvcommon", .spirv },
    .{ "spirvcl", .spirv },
    .{ "spirvvk", .spirv },
});
...
const MacroIterator = struct {
    ...
    pub fn next(self: *MacroIterator) ?[]const u8 {
        ...
        if (value[0] == '"') {
            ...
            const end_index = std.mem.findScalar(u8, self.token_it.buffer[self.token_it.index ..], '"').? + 1;
            self.token_it.index += end_index;
            const new_value = value.ptr[0 .. value.len + end_index];
            // std.debug.print("value '{s}' new_value '{s}' it.buffer[it.index..] '{s}'\n", .{ value, new_value, self.token_it.buffer[self.token_it.index..] });
            return new_value;
        }
        return value;
    }
...
const DafsaBuilder = struct {
    ...
    fn writeDafsa(self: *const DafsaBuilder, writer: anytype) !void {
        ...
        first_available_index = try self.writeDafsaChildren(self.root, writer, &queue, &child_indexes, first_available_index);

        while (queue.items.len > 0) {
            // TODO: something with better time complexity
            const node = queue.orderedRemove(0);

            first_available_index = try self.writeDafsaChildren(node, writer, &queue, &child_indexes, first_available_index);
        }
